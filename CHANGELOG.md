...
- **Your settings and presets follow your account.** Written to Lumiverse under your account and read back on load, so a different browser or a different machine finds the setup you left rather than a fresh install. The browser keeps a copy as a cache, which is what draws the panel instantly and what carries on working offline. On a server with several accounts, each account's settings and presets are stored separately and one account cannot read another's. A save that fails says so, rather than letting settings look saved when they are not.
- **The floating button's menu is Lumiverse's own.** Holding the button, or right clicking it, opens a menu with the tab, a refine, an undo when there is one, the automatic pass, the per chat switch, a way to hide the button, and the master switch, anchored under the button. Anything that would otherwise be a row in the chat input's Extras menu lives in this menu while the button is on screen, and moves back to Extras when it is not. Two ways to reach one thing is one more than anybody needs.
- **A refine is dropped if the reply moved while it was being written.** The message is read again before the answer is saved, and a message that changed in the meantime is left alone. Somebody else's edit is worth more than a refine, and Auto Retry's word swaps land in exactly that window.
- **The output format is the prompt's to write.** Asking for the answer between `<refined>` tags is written out in the How to answer block, in plain words in a text box, rather than filled in by a macro you could not see or reword. Nothing outside those tags is ever saved into a chat, so a prompt is free to ask there for a report on what was cut, added, or deliberately left alone; what comes back appears on the Log tab under **What it said about the edit**.
