...
// and once for a model that reasons. name is what it is stored and looked up as
// and has to be unique across both sets; label is what the list shows, which can
// repeat because the heading above it already says which prompt it is for.
type Shipped = {
  name: string;
  label: string;
  mine: boolean;
...
export function setup(ctx: Ctx, overrides?: any) {
  ...
  function buildBlocksCard(): HTMLElement {
    ...
    if (shippedMoved()) {
      ...
      const what = el(
        "span",
        "",
        "The prompts that ship with Auto Refine have changed since you last loaded one. Yours is untouched. To take the new wording, load one from the card below, which writes over the list you are on.",
      );
      what.style.flex = "1";
  ...
  function buildPresetCard(): HTMLElement {
    ...
    if (presetPick && isBuiltIn(presetPick)) {
      ...
      wrap.appendChild(
        note(
          "One of the ones that ship with the extension. Picking it loaded it, so change it however you like and save it under a name of your own.",
        ),
      );
    }
    if (presetSaid) {
