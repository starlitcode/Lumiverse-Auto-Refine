...
export function setup(ctx: Ctx, overrides?: any) {
  ...
  function putBack(held: Array<{ node: any; at: number }>) {
    for (const one of held) {
      try {
        if (one.node && one.node.scrollTo && one.node === (globalThis as any))
          one.node.scrollTo(0, one.at);
        else if (one.node) one.node.scrollTop = one.at;
      } catch (_) {}
    }
  }
  ...
  function buildGuardCard(): HTMLElement {
    ...
    for (const f of GUARD_FIELDS)
      if (fieldShows(f) && !f.under) wrap.appendChild(fieldRow(f));
  ...
  function showLive(text: string): void {
    live = text;
    try {
      if (!liveWatch) {
        paint();
  ...
  function splitLive(text: string): { notes: string; body: string } {
    const t = String(text || "");
    let notes = "";
    let body = "";
  ...
  function buildNotesCard(): HTMLElement | null {
    ...
    const wrap = card(
      ...
      "Whatever the model wrote outside the <REFINED> tags on the last pass. None of it was saved into your chat. The two reasoning prompts ask for working here, between <REFINE_NOTES> tags; on the other two this stays empty until you ask for something in the How to answer block.",
  ...
  function buildChatCard(): HTMLElement {
    ...
    if (sweep) {
      wrap.appendChild(
        note(
          "Refining reply " + sweep.at + " of " + sweep.of + ". " +
      ...
      wrap.appendChild(
        note(
          "Refines every reply in this chat, oldest first, one call each. The greeting is left alone, and so are your own messages. Each one can be put back from the Log afterwards, and Stop ends the run.",
        ),
      );
