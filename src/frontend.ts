...
// Four that ship with it: a line edit and a copy edit, each once for any model
...
export function setup(ctx: Ctx, overrides?: any) {
  ...
  if (!cfg.shippedSeen) cfg.shippedSeen = SHIPPED_MARK;
  ...
  function markShippedSeen() {
    if (cfg.shippedSeen === SHIPPED_MARK) return;
  ...
  function loadPreset(p: Preset, wasPick?: string): void {
    ...
    // is not.
    if (took && isBuiltIn(p.name)) markShippedSeen();
    let alsoSaid = "";
