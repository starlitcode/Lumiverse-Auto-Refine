...
const PERMS: Array<{ id: string; label: string; why: string; without: string; fatal?: boolean }> = [
  {
    id: "generation",
    label: "Generation",
    why: "Runs the refine.",
  ...
  },
  {
    ...
    without:
      "Refines still run and cost you a model call, and nothing can be written, so nothing changes.",
    fatal: true,
  },
  {
    id: "chats",
...
// of them. Left inside Hand It In, that check could never fire, because that
// block always has prose in it: the tag pair would be sent empty every time.
//
// Last, under Hand It In, which is where the macro already sat. So what a
...
const COPY_EXACTLY: Block = {
  ...
  text:
    ...
    "- links, image links and file paths\n- stat blocks, status bars, " +
    "trackers, inventories, timestamps, and any line printed to the same " +
    "shape every time\n- a second language beside the first, with the line " +
...
const YOURS_DEFAULT: Block[] = YOURS_LONG;
...
const DEFAULT_BLOCKS: Block[] = PLAIN_LONG;

// Four that come with it: the judge and the line judge, each once for any model
...
const BUILT_IN_PROMPTS: BuiltIn[] = [
  {
    name: "A judge",
    label: "A judge",
    mine: false,
  ...
  },
  {
    ...
    thinking: "inherit",
    what: "The same job, worked out rather than listed: one call for telling a mistake from a choice, the hot spots, a scorecard, and what is never a repair. Needs a model that reasons.",
  },
];
...
const OLD_BUILT_IN_NAMES: Record<string, string> = {
...
const linesOf = (text: any): string[] =>
  String(text == null ? "" : text)
    .split("\n")
    .map((x) => x.trim())
    .filter(Boolean);

const renamedBuiltIn = (name: any): any => {
...
function markText(text: string): string {
  let h = 0x811c9dc5;
...
function holdRingSvg(): string {
  return (
    '<svg class="arf-hold" viewBox="0 0 100 100" aria-hidden="true" focusable="false">' +
...
export function setup(ctx: Ctx, overrides?: any) {
  ...
  function carryOldNames(into: any) {
    ...
    for (const k of ["presetPick", "presetPickYours"]) if (k in into) into[k] = renamedBuiltIn(into[k]);
    if (Array.isArray(into.passNames)) into.passNames = into.passNames.map(renamedBuiltIn);
    return into;
  }
  ...
  if (!cfg.builtInSeen) cfg.builtInSeen = BUILT_IN_MARK;
  ...
  // built-in prompt into storage on a fresh install would make every later
  ...
  function markBuiltInSeen() {
    if (cfg.builtInSeen === BUILT_IN_MARK) return;
  ...
  // is this browser's copy, which can be older than the account's, and a phone
  // opened after the prompt was changed on a computer would write its old
  // prompt over the new one. The account copy is asked for straight after, and
  // wins.
  function armBackend() {
    send({ type: "set_settings", settings: forBackend(), keep: true });
  ...
  disposers.push(() => {
    if (logPaintSoon) clearTimeout(logPaintSoon);
    logPaintSoon = null;
  });
  ...
  const outsideAnyChat = (): boolean => noChatOpen && lastChatId == null && idInUrl() == null;
  ...
  type Slot = { at: number; after: string | null };
  let urlSlot: Slot | null = (() => {
    try {
  ...
  function learnSlot(id: any) {
    const t = id == null ? "" : String(id);
    if (t.length < URL_ID_MIN) return;
    const parts = urlParts();
    const at = parts.indexOf(t);
    if (at < 0) return;
    const next: Slot = { at: at, after: at > 0 ? parts[at - 1] : null };
    if (urlSlot && urlSlot.at === next.at && urlSlot.after === next.after) return;
    urlSlot = next;
    try {
  ...
  function idInUrl(): string | null {
    if (!urlSlot) return null;
    const parts = urlParts();
    const got = parts[urlSlot.at];
    if (!got || got.length < URL_ID_MIN) return null;
    ...
    if (urlSlot.after != null && parts[urlSlot.at - 1] !== urlSlot.after) return null;
    return got;
  }
  ...
  // is still reported here, and taking it as the chat you are in would light
  // every button on the home screen.
  ...
  //          is none.
  let chatAskWhy: "who" | "guess" | "where" = "where";
  ...
  const tokenWord = (n: number, counted: boolean): string =>
    (counted ? "" : "roughly ") + n.toLocaleString() + " tokens";
  ...
  const hasPerm = (id: string) => !granted || granted.indexOf(id) >= 0;
  function missing(): typeof PERMS {
    const have = granted;
    if (!have) return [];
  ...
  let sweepWatch: any = null;
  function armSweepWatch() {
    if (sweepWatch) clearTimeout(sweepWatch);
    sweepWatch = null;
    const cap = waitCap();
    if (!cap) return;
    sweepWatch = setTimeout(
      () => {
        ...
        sweepAsk = null;
        log(
          "the run through the chat stopped answering after reply " + got.at + " of " + got.of,
        );
  ...
  const isTemporary = (id: any) => id != null && cardless.has(String(id));
  ...
  const holdsTurn = (list: Block[]) =>
    list.some((b) => b.on && String(b.text || "").indexOf(TURN_MACRO) >= 0);
  ...
  function placeFixed(el2: any, left: number, top: number) {
  ...
  const fromMouse = (e?: any): boolean => {
    ...
    if (t) return t === "mouse";
    if (lastPointer) return lastPointer === "mouse";
  ...
  function hintButton(text: string | undefined, label: string): HTMLElement | null {
    ...
    q.addEventListener("mouseenter", (e: any) => {
      if (fromMouse(e)) open();
    });
    q.addEventListener("mouseleave", (e: any) => {
      if (fromMouse(e)) shut();
    });
    ...
    q.addEventListener("click", (e: any) => {
  ...
  if (typeof document !== "undefined") {
    ...
    const onDown = (e: any) => {
      ...
      try {
        ...
        if (t && t.closest && (t === hintAnchor || t.closest("[data-arf-hint]"))) return;
  ...
  function shownText(node: any): string {
    if (!node) return "";
    ...
    for (let i = 0; i < kids.length; i++) out += shownText(kids[i]) + " ";
    return out;
  }
  ...
  function scrollers(root: any): Array<{ node: any; at: number }> {
    ...
    try {
      ...
      if (w && w.scrollY > 0) out.push({ node: w, at: w.scrollY });
    } catch (_) {}
    return out;
  }
  ...
  // back is what to fall back to when the thing being held is the thing about to
  // go. Pressing Delete on a block holds that block, which the rebuild has no
  // copy of, so the correction found nothing to measure and did not run at all:
  // the panel was left wherever the plain scroll number happened to land. The
  // nearest marked thing above survives the delete, and it is also what the
  // reader is looking at, since what they pressed is on its way out.
  type Anchor = { sel: string; nth: number; at: number; back?: Anchor };
  ...
  function markOf(root: any, node: any): Anchor | null {
  ...
  function backOf(root: any, node: any): Anchor | null {
    try {
      const all = root.querySelectorAll(ANCHOR_MARKS.map((m) => "[" + m + "]").join(","));
      let at = -1;
      for (let i = 0; i < all.length; i++) if (all[i] === node) { at = i; break; }
      for (let i = at - 1; i >= 0; i--) {
        const n: any = all[i];
        if (n.contains && n.contains(node)) continue;
        const sel = anchorSel(n);
        if (!sel || root.querySelectorAll(sel).length !== 1) continue;
  ...
  function reAnchor(root: any, a: Anchor | null, held: Array<{ node: any; at: number }>) {
    ...
    try {
      ...
      let use: Anchor = a;
      let n: any = root.querySelectorAll(a.sel)[a.nth];
  ...
  function putBack(held: Array<{ node: any; at: number }>, onlyShort?: boolean) {
    const w: any = globalThis as any;
    for (const one of held) {
      try {
        const node = one.node;
        ...
        if (now === one.at) continue;
        if (onlyShort && !(now < one.at)) continue;
        if (node === w && node.scrollTo) node.scrollTo(0, one.at);
        else node.scrollTop = one.at;
      } catch (_) {}
    }
  }
  ...
  function paint() {
    ...
    const back = undoHere();
    const mine = draftBack();
    if (mine && (!back.length || mine.at >= back[0].at)) root.appendChild(buildLastDraft(mine));
  ...
  type DiffPart = { how: -1 | 0 | 1; text: string };
  ...
  function diffColumn(lab: string, whole: string, parts: DiffPart[] | null, keep: -1 | 1): HTMLElement {
    ...
    if (!parts) well.appendChild(el("span", "", whole));
    else
      for (const p of parts) {
        if (p.how === 0) well.appendChild(el("span", "", p.text));
        else if (p.how === keep)
  ...
  function fillDiff(wrap: HTMLElement, before: string, after: string) {
    ...
    if (side) {
      ...
      if (big) wrap.appendChild(el("div", "arf-note", "Too long to mark up, so both are shown whole."));
      return;
    }
  ...
  function diffWell(before: string, after: string): HTMLElement {
    ...
    if (diffSpots.length > 40) diffSpots.splice(0, diffSpots.length - 40);
    return wrap;
  }
  ...
  function diffSwitch(): HTMLButtonElement {
  ...
  function changedHead(label: string): HTMLElement {
  ...
  function buildLastDraft(one: { before: string; after: string; at: number }): HTMLElement {
    const wrap = card(
      "Your draft, refined",
      "Only while the box still holds it. Type over it and there is nothing to put back.",
      new Date(one.at).toTimeString().slice(0, 5),
    );
  ...
  function fieldRow(f: Field): HTMLElement {
    ...
    if (f.type === "bool") {
      ...
      const isList = Array.isArray((CONFIG as any)[f.key]);
  ...
  function buildBlocksCard(): HTMLElement {
    ...
    if (pillEl) pillEl.setAttribute("data-arf-blockcount", "1");
    ...
    if (builtInMoved()) {
      ...
      const what = el(
        "span",
        "",
        "The built-in prompts have changed since you last loaded one. Yours is untouched. To take the new wording, load one from the card below, which writes over the list you are on.",
      );
      what.style.flex = "1";
  ...
  function buildSetupCard(): HTMLElement {
    ...
    if (setupUndo) row.appendChild(putBack);
    ...
    if (one) {
      const named = (id: string) => {
        if (!id) return "the model you are chatting with";
      ...
      if (setupLost(one))
        wrap.appendChild(
          warn(
            "The connection this setup names is not on your account any more. Loading it would leave the refine pointed at nothing, so pick another connection above and update the setup.",
          ),
        );
    }
    if (setupSaid) wrap.appendChild(note(setupSaid));
    return wrap;
  }
  ...
  function samplerRow(s: {
    ...
    wrap.appendChild(box);
    if (s.hint) (wrap as any)._arfHint = s.hint;
    return wrap;
  }
  ...
  function buildNotesCard(): HTMLElement {
    const wrap = card(
      "What the model worked out",
      undefined,
      keptNotes ? (keptNotes.ok ? "saved" : "dropped") : undefined,
    );
    if (!keptNotes) {
      wrap.appendChild(
        note(
  ...
  function debugText(): string {
    ...
    if (on("prompt")) {
      ...
      if (!holdsTurn(yours)) lines.push("  NOTE: the own-messages prompt carries no {{message}}");
    }
  ...
  function buildPermsCard(): HTMLElement {
    const gone = missing();
    const wrap = card(
      "What it is allowed to do",
      undefined,
      granted ? (gone.length ? gone.length + " refused" : "all granted") : "asking",
    );
    if (!granted) {
    ...
    if (!gone.length) {
      wrap.appendChild(
        notice("good", "Everything it asks for is granted, so nothing here is held back."),
      );
      return wrap;
    }
    ...
    wrap.appendChild(
      note(
        "Grant any of these in Lumiverse's own extension settings. Nothing here needs a reload: the panel notices the moment one changes.",
      ),
    );
    wrap.appendChild(
      fold("What each one is for", (body) => {
  ...
  function buildReachCard(): HTMLElement {
    ...
    if (cfg.widgetOn && widgetFailed)
      wrap.appendChild(
        bad(
          "The floating button could not be created. Check that the interface panels permission is granted.",
        ),
      );
    ...
    for (const f of WIDGET_FIELDS) wrap.appendChild(fieldRow(f));
    wrap.appendChild(
      fieldRow({
        key: "inputRefine",
  ...
  // ---- model setups ----
  // A named set of the Model tab: the connection, the thinking, the wait and
  // the samplers. Presets keep the prompt; these keep what runs it, so the same
  // prompt can be sent to a cheap model one minute and a good one the next
  // without either of them being edited to do it.
  type Setup = { name: string; at: number; settings: Record<string, any> };
  let setups: Setup[] = [];
  let setupPick = "";
  let setupName = "";
  ...
  let setupUndo: { settings: Record<string, any>; pick: string } | null = null;
  let setupSaid: string | null = null;

  function cleanSetups(list: any): Setup[] {
  ...
  function tookAccountSetups(msg: any): void {
    if (!msg || msg.requestId !== setupAsk) return;
    setupAsk = "";
    ...
    if (clean.length) {
      ...
      log(
        "brought " + clean.length + (clean.length === 1 ? " model setup" : " model setups") + " down from your account",
        true,
      );
      paint();
      ...
      saveSetups();
      log(
        "sent " + setups.length + (setups.length === 1 ? " model setup" : " model setups") + " up to your account",
        true,
      );
    }
  }
  ...
  function applySetup(one: Setup): number {
    let took = 0;
    for (const k of SETUP_KEYS) {
      if (!(k in one.settings)) continue;
  ...
  function setupLost(one: Setup): boolean {
  ...
  function presetFromNow(): Record<string, any> {
    ...
    for (const k of PRESET_KEYS) {
      if (k === "blocks") out.blocks = blockList("blocks").map((b) => ({ ...b }));
      else if (k === "userBlocks") out.userBlocks = blockList("userBlocks").map((b) => ({ ...b }));
  ...
  function loadPreset(p: Preset, wasPick?: string): void {
    ...
    // is not.
    if (took && isBuiltIn(p.name)) markBuiltInSeen();
    let alsoSaid = "";
  ...
  function loadSetup(one: Setup, wasPick?: string): void {
  ...
  function buildPresetCard(): HTMLElement {
    ...
    for (const one of allPresets()) {
      ...
      if (known && known.mine !== editingYours()) continue;
      if (known) groups[known.mine ? 1 : 0].of.push(one);
    ...
    const driftedFromPick = () => {
      ...
      try {
        ...
        const now: Record<string, any> = presetFromNow();
        const other = editingYours() ? "blocks" : "userBlocks";
        for (const k of Object.keys(p.settings)) {
          if (k === other) continue;
          if (steady(now[k]) !== steady(p.settings[k])) return true;
        }
        return false;
      } catch (_) {
        return false;
    ...
    const builtInSaid = note(
      "A built-in prompt cannot hold this. Press Save as new and the copy keeps it.",
    );
    ...
    if (currentPick() && isBuiltIn(currentPick())) {
      ...
      wrap.appendChild(
        note(
          "One of the four built in. Picking it loaded it, so change it however you like and save it under a name of your own.",
        ),
      );
    }
    if (presetSaid) {
  ...
  function raiseWidget(at?: { x: number; y: number } | null) {
    ...
    try {
      ...
      b.addEventListener("contextmenu", (e: any) => {
        try {
          e.preventDefault();
        } catch (_) {}
        disarm();
  ...
  async function widgetMenu() {
    ...
    else if (picked === "snip") snipPicked();
  ...
  function syncExtras() {
    ...
    extra("auto-refine-snip", "Take out what I selected", inExtras && !!pickedHere(), () =>
      snipPicked(),
    );
    ...
    try {
      ...
      if (wantMsg) {
        ...
        for (const id in picked) {
          ...
          const whole = into.querySelector('[data-arf-slot="message"]') as HTMLElement | null;
          if (whole) whole.style.display = mine ? "none" : "";
      ...
      const off = ctx.onBackendMessage((msg: any) => {
        try {
          ...
          if (msg.type === "backend_ready") {
            ...
            // the account's.
            if (accountAsk) loadFromAccount();
            if (presetAsk) loadPresetsFromAccount();
          ...
          if (msg.type === "permissions") {
            if (permsAsk && msg.requestId !== permsAsk) return;
            permsAsk = null;
          ...
          if (msg.type === "active_chat") {
            ...
            if (msg.chatId) {
              ...
              if (chatAskWhy === "guess" && !msg.found) return;
            ...
            if (msg.chatId != null && msg.canUndo)
              {
                ...
                showPop(undoable.get(k) as Undo);
              }
          ...
          if (msg.type === "refine_stopped") {
            ...
            if (!msg.stopped) log("there was nothing running to stop");
            markBusy(false);
          ...
          if (msg.type === "undo_result") {
            ...
            if (msg.ok) {
              ...
              if (msg.gone && about) {
                ...
                if (popKey === undoKey(about.chatId, about.messageId)) dropPop();
                if (!undoHere().length) setBadge(null);
          ...
          if (msg.type === "refine_all_progress") {
            if (sweepAsk && msg.requestId !== sweepAsk) return;
            sweep = {
              at: Number(msg.at) || 0,
              of: Number(msg.of) || 0,
          ...
          if (msg.type === "refine_all_done") {
            ...
            if (why) log("what stopped the rest: " + why);
