...
function markupOut(text: string): string {
...
function trackerLine(line: string): boolean {
  const t = String(line).replace(/[*_]/g, '').trim();
  if (!t) return false;
  if ((t.match(/\|/g) || []).length >= 2) return true;
  if (/^[\[{].*[\]}]$/.test(t)) return true;
  const label = /^([^:"\u201c\n]{1,30}):\s*\S/.exec(t);
  if (label && label[1].trim().split(/\s+/).length <= 3) return true;
  return false;
}
...
let extraPairs: Array<[string, string[]]> = [];

function setPairs(raw: any): void {
  ...
  for (const line of String(raw == null ? '' : raw).split(/\n/)) {
    ...
    if (!soft || !blunt) continue;
    const had = out.filter((x) => x[0] === soft)[0];
    if (had) {
      if (had[1].indexOf(blunt) < 0) had[1].push(blunt);
    } else {
      out.push([soft, [blunt]]);
    }
    if (out.length >= 200) break;
...
function swappedIn(original: string, rewrite: string): string[] {
  if (!guardSoften) return [];
  ...
  for (const [soft, blunts] of SOFT_PAIRS.concat(extraPairs)) {
    ...
    if (!saysIt(now, soft) || saysIt(was, soft)) continue;
    for (const blunt of blunts) {
      // And gone: the reply said it, the rewrite does not.
      if (!saysIt(was, blunt) || saysIt(now, blunt)) continue;
...
const nextAutoAt = new Map<string, number>();
...
function judgeInner(answer: any, original: string): Verdict {
  ...
  if (swapped.length)
    return {
      ...
      text: '',
      why:
        'the rewrite softened the reply, putting ' +
...
async function gatherPersonaName(
  chatId: string,
  characterId: string,
  userId?: string,
): Promise<string> {
  try {
    if (!spindle.macros || typeof spindle.macros.resolve !== 'function') return '';
...
async function countSaying(
  ...
  try {
    ...
    if (got && got.approximate) return { n: guess, counted: false };
    const n = got && Number(got.total_tokens);
...
function fromTheStory(phrase: string, known: string): boolean {
  const words = phrase.split(' ');
  if (words.length < PHRASE_MIN) return known.indexOf(phrase) >= 0;
  for (let i = 0; i + PHRASE_MIN <= words.length; i++)
    if (known.indexOf(words.slice(i, i + PHRASE_MIN).join(' ')) >= 0) return true;
  return false;
}
...
function gatherWorn(msgs: any[], upTo: number, name?: string, canon?: string): string {
  ...
  // writing their story.
  const known = canon ? wordsOf(String(canon), new Set<string>()).join(' ') : '';
  const lines: string[] = [];
  for (const one of worn) {
    ...
    if (known && fromTheStory(one.phrase, known)) continue;
...
async function askModel(
  ...
  try {
    const req: any = { messages: await buildPrompt(text, isUser, scene, userId, undefined, use) };
...
async function refineMessage(
  ...
  const chain: Pass[] =
    manyPasses && passList.length ? passList : [{ name: 'the prompt', blocks: activeBlocks(isUser) }];
  ...
  const youName =
    nameSpeakers && promptWants(HISTORY_MACRO, isUser, willSend)
...
async function swipeContent(chatId: string, messageId: any, at: number): Promise<string | null> {
  try {
    const msgs = await spindle.chat.getMessages(chatId);
    if (!Array.isArray(msgs)) return null;
    ...
    if (!listOn || !Array.isArray(m[listOn]) || at >= m[listOn].length) return null;
...
function onFreshThinking(original: string, next: string, fresh: string): string | null {
  const was = splitThinking(original);
  const now = splitThinking(fresh);
  if (now.body !== was.body) return null;
  if (!next.startsWith(was.head) || !next.endsWith(was.tail)) return null;
  if (next.length < was.head.length + was.tail.length) return null;
...
async function saveRefined(
  ...
  try {
    const fresh = aimed ? await swipeContent(chatId, m.id, onSwipe as number) : await currentContent(chatId, m.id);
    ...
    const idx = aimed ? (onSwipe as number) : m && typeof m[atOn] === 'number' ? m[atOn] : 0;
...
function jevOutputText(output: any): string | undefined {
  ...
  for (const item of output) {
    ...
    for (const part of parts) if (part && part.type === 'output_text' && typeof part.text === 'string') return part.text;
  }
  return undefined;
}
...
function cleanJevKey(raw: any): string {
...
const sayQueued = (userId?: string) => () =>
  tell(userId, { type: 'refine_progress', stage: 'queued', waitMs: TURN_BEAT_MS * 2 });
...
async function ownerOf(p: any): Promise<string | undefined> {
  if (p && p.userId) return String(p.userId);
  const keys = Array.from(rulesBy.keys());
  if (keys.length <= 1) return keys.length ? keys[0] || undefined : settingsUser;
  const hits: string[] = [];
  for (const k of keys) {
    try {
  ...
  if (hits.length === 1) return hits[0] || undefined;
...
function applyRules(s: any): void {
  ...
  wornFine = Array.isArray(s.wornFine)
    ? s.wornFine
        .map((x: any) => wordsOf(String(x == null ? '' : x), new Set<string>()).join(' '))
        .filter(Boolean)
    : [];
  ...
  passList = Array.isArray(s.passes)
    ? s.passes
        .filter((x: any) => x && x.on !== false && Array.isArray(x.blocks) && x.blocks.length)
        .map((x: any, i: number) => ({
          name: String(x.name == null || !String(x.name).trim() ? 'Pass ' + (i + 1) : x.name),
          blocks: x.blocks as Block[],
        }))
        .filter((x: Pass) =>
          x.blocks.some(
            (b: any) => b && b.on !== false && String(b.text || '').indexOf(TURN_MACRO) >= 0,
          ),
        )
        .slice(0, CHAIN_MAX)
    : [];
...
async function onPanel(payload: any, userId?: string): Promise<void> {
  try {
    ...
    if (payload.type === 'set_chats_off') {
      ...
      const off = new Set<string>(list.slice(0, 500).map((c: any) => String(c)));
      const k = keyOf(userId);
      chatsOffBy.set(k, off);
      if (turnCount === 0 || turnKey === k) chatsOff = off;
      return;
    }
...
try { spindle.sendToFrontend({ type: 'backend_version', version: VERSION }); } catch (_) {}

try { spindle.log.info('Auto Refine ' + VERSION + ' backend loaded.'); } catch (_) {}
