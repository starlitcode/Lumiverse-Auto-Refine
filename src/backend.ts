...
function cacheFacts(meta: any): { on: boolean | null; ttl: string } {
  const out: { on: boolean | null; ttl: string } = { on: null, ttl: '' };
  ...
  try {
    for (const key of Object.keys(meta)) {
      ...
      if (/ttl|expiry|expires|duration/i.test(key)) {
        ...
        if (said && said.length < 12 && /[^0-9]/.test(said)) out.ttl = said;
        continue;
      }
      ...
      if (/tool|system|prefix|conversation|message|breakpoint|auto/i.test(key)) continue;
