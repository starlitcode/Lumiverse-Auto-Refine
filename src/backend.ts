...
const JEV_HOSTS: Record<string, { url: string; model: string }> = {
...
function jevWhere(): { url: string; model: string } {
  if (judgeHost === 'custom') return { url: judgeUrl, model: judgeModel };
...
): Promise<{ answers?: any; cost?: number; error?: string }> {
...
async function judgeReply(userId: string | undefined, reply: string, worn: string): Promise<JevVerdict> {
  ...
  if (!scores.length) return { refine: true, failed: true, why: 'Jev sent back no usable answers', cost: got.cost };
