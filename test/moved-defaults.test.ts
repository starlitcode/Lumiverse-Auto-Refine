...
// the table behind it. A row whose old value is also the value it has now tells
...
// Auto Retry carries the same check against its own table.
...
describe("the defaults said to have moved", () => {
  ...
  test("and an old value that is not the one it has now", () => {
    // A row that matched the current value would put the line in front of
  ...
  test("the label is the one on the row it points at", () => {
    ...
    for (const m of MOVED_DEFAULTS)
      expect(labels.some((l: string) => l === m.label || l.indexOf(m.label) === 0)).toBe(true);
  });
  ...
  test("every moved setting can still leave the panel", () => {
    // A row offering a new value for a setting nothing exports would hand
    // somebody a number that does not travel with the rest of their setup.
    const { PARTS } = __testing as any;
