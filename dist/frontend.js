...
function deriveInputSelector(start) {
    let node = start;
    let hops = 0;
    // A hold can land on a wrapper around the box rather than the box itself.
    while (node && hops < 5) {
        const tag = String(node.tagName || "").toLowerCase();
        if (tag === "textarea" || tag === "input" || node.isContentEditable === true)
            break;
    ...
    for (const attr of ["name", "data-testid", "data-test-id", "aria-label", "placeholder", "id"]) {
        ...
        if (!v || !String(v).trim())
            continue;
        if (attr === "id" && !(SAFE_NAME.test(v) && !UNSTABLE_NAME.test(v)))
            continue;
        if (attr === "id")
            return "#" + v;
    ...
    if (comp)
        return "[data-component=" + q(comp) + "] " + tag;
...
export function setup(ctx, overrides) {
    ...
    function onHoldStart(e) {
        ...
        try {
            if (e.target && e.target.closest && e.target.closest(".arf"))
                return;
        }
        catch (_) { }
    ...
    function buildInputCard() {
        ...
        function paintList() {
            ...
            list.appendChild(el("div", "arf-note", blank
