...
export function setup(ctx, overrides) {
    ...
    function onAShippedPrompt() {
        const mine = (list) => list.map((b) => String(b.text)).join("\u0000");
    ...
    function buildPresetCard() {
        ...
        if (!onAShippedPrompt() && String(cfg.promptsSeen || "") !== shippedMark()) {
            ...
            box.appendChild(note("The prompts that ship with Auto Refine have changed since the ones you are carrying. Yours are kept as they are, edits and all, so loading one below is the only way to take the new wording. Load the one for replies and the one for your own writing separately."));
