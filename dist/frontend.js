...
export function setup(ctx, overrides) {
    ...
    const asksForWorking = () => blockList().some((b) => b.on && /<\s*refine_notes\s*>/i.test(String(b.text || "")));
    function aboutWorking() {
        ...
        return ("It does not ask the model for its working, so there is nothing to watch while it writes. The two for a model that thinks ask for it, and so does any block of your own with " +
            NOTES_TAG +
            " in it.");
    }
    ...
    function addUndoButton(bar, id, after) {
        try {
            const had = bar.querySelector('[data-arf-undo="' + id + '"]');
            if (!undoableHere(id)) {
                if (had)
                    had.remove();
                return;
            ...
            if (after.parentNode === bar && after.nextSibling)
                bar.insertBefore(u, after.nextSibling);
            else
                bar.appendChild(u);
        }
        catch (_) { }
