...
export function setup(ctx, overrides) {
    ...
    function buildPromptShape() {
        ...
        if (!shapeOpen)
            return wrap;
        wrap.appendChild(note("The refine is one request, and this is what goes in it and in what order. Blocks next to each other with the same role are sent as one message. A block with nothing to say is left out."));
    ...
    function buildFold() {
        ...
        wrap.appendChild(note("Left blank, the connection's own preset decides, which is what you want unless you have a reason. Fill one in and it is sent with the refine, and only with the refine: your chat is not affected."));
        for (const s of SAMPLER_FIELDS)
            wrap.appendChild(samplerRow(s));
    ...
    function buildTransfer() {
        ...
        wrap.appendChild(note("A file with your rules, your prompt layout and your sampler settings in it. Importing replaces what you have here, so export first if you want a way back."));
        const row = el("div", "arf-row");
