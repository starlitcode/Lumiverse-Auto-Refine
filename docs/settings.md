...
- **Saved model setups** keeps everything on this tab under a name: the connection, the thinking, the wait, the samplers and the two prices. One is there before you save anything: **Lower temperature, for rewriting**, which carries a temperature of 0.7 and nothing else. It names no connection and no prices, so loading it cannot move you off the model you picked or put a figure on the panel that is not yours, and every prompt that ships with the extension brings it along so one pick is the whole arrangement. It cannot be renamed or deleted, since it would be back on the next update. Save it as new under a name of your own and that copy is yours to change. Save one for a cheap model and one for a careful one, and move between them in a tap. The prices travel with the rest because they belong to the model: a setup that swapped the model and left the old prices standing would put a figure on the panel that was true of neither. A setup you saved before the price boxes existed carries none, and loading it leaves the ones you have alone.
...
- **Refine a reply that has been refined before** decides whether the automatic pass returns to a reply it has already been through. Off by default, so each reply is refined once. It only appears while the automatic pass is on, and it changes nothing about the buttons: pressing one always refines what you pressed it on. Two announcements of one generation are still one reply either way, so a build that reports a reply as finished twice cannot get it refined twice.
