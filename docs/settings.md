...
- **Seconds between automatic refines** spaces out the automatic pass, for a provider that meters calls per minute. A reply that lands inside the gap waits for it and is then refined, with the seconds counting down in the panel. It is 0 by default, which is no gap. Refines you start yourself never wait for it.
