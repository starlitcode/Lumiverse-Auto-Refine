...
Auto Refine lives in a tab in Lumiverse's sidebar drawer, not behind a settings window. Open it from the drawer, from **Auto Refine** in the chat input's Extras menu, or from `Ctrl+K`.
...
- **Show the refine button in Extras** adds a one-tap way to refine the latest reply without opening the tab, which is the route that works on a phone.
