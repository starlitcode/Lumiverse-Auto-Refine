...
- **Refine a reply that has been refined before** decides whether the automatic pass returns to a reply it has already been through. Off by default, so each reply is refined once. On, a reply you swiped or regenerated is refined again. It only appears while the automatic pass is on, and it changes nothing about the buttons: pressing one always refines what you pressed it on. Two announcements of one generation are still one reply either way, so a build that reports a reply as finished twice cannot get it refined twice.
